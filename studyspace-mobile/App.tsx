...
const overlaps = (bookings: Booking[], start: number, end: number) =>
  bookings.some((booking) => start < booking.end && end > booking.start);

const availableOptions = (room: Room) =>
  options.filter((option) => !overlaps(room.bookings, option.start, option.end));
...
const roomStatus = (room: Room): Status => {
  ...
  if (overlaps(room.bookings, NOW, NOW + 1)) return 'current';
  return 'free';
};

const availabilityText = (room: Room) => {
  const available = availableOptions(room);
  if (available.length === 0) return 'Heute vollständig ausgebucht';
...
const optionFor = (booking: Booking | null) =>
  options.find((option) => booking?.start === option.start && booking.end === option.end);
...
export default function App() {
  ...
  const freeRoomCount = rooms.filter((room) => roomStatus(room) === 'free').length;
  ...
  const bookRoom = () => {
    ...
    setRooms((current) =>
      current.map((room) =>
        room.id === selectedRoom.id ? { ...room, bookings: [...room.bookings, booking] } : room,
      ),
    );
      ...
      {screen === 'rooms' && (
        <ScrollView style={styles.screen} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            <Text style={styles.kicker}>Campus heute · Jetzt {time(NOW)} Uhr</Text>
          ...
          <View style={styles.legendRow}>
            ...
            <LegendDot color={colors.gray} label="gerade belegt" />
          ...
          <View style={styles.roomList}>
            {rooms.map((room) => {
              ...
              const label = status === 'free' ? 'Frei' : status === 'current' ? 'Gerade belegt' : 'Heute ausgebucht';
              return (
                <TouchableOpacity
                  key={room.id}
                  activeOpacity={0.85}
                  style={[styles.roomCard, status === 'current' && styles.currentCard, status === 'full' && styles.fullCard]}
                ...
                >
                  ...
                  <Text style={styles.availabilityText}>{availabilityText(room)}</Text>
                  <View style={styles.chipRow}>
                    {room.equipment.slice(0, 3).map((item) => <Chip key={item} label={item} />)}
                  </View>
                </TouchableOpacity>
              );
            })}
      ...
      {screen === 'detail' && selectedRoom && (
        <ScrollView style={styles.screen} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.detailHero}>
            <StatusBadge
              status={roomStatus(selectedRoom)}
              label={roomStatus(selectedRoom) === 'free' ? 'Jetzt frei' : roomStatus(selectedRoom) === 'current' ? 'Gerade belegt' : 'Heute ausgebucht'}
            />
          ...
          <View style={styles.section}>
            ...
            {currentAvailableOptions.length === 0 ? (
              <View style={styles.emptySlots}><Text style={styles.emptySlotsText}>Dieser Raum ist heute vollständig ausgebucht.</Text></View>
            ) : (
                ...
                {options.map((option) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[styles.bookingOption, active && styles.bookingOptionActive, !available && styles.bookingOptionDisabled]}
      ...
      {screen === 'success' && bookedRoom && bookedSlot && (
        <View style={styles.successScreen}>
          ...
          <Text style={styles.successSubtitle}>
            {bookedRoom.name} ist {optionFor(bookedSlot)?.title.toLowerCase()} von {time(bookedSlot.start)} bis {time(bookedSlot.end)} Uhr für dich reserviert.
          </Text>
          ...
          {bounces.map((bounce, index) => (
            <Animated.View
              ...
              style={[styles.loadingDot, {
                opacity: bounce.interpolate({ inputRange: [0, 1], outputRange: [0.45, 1] }),
...
function UniIcon({ animation, label, type, featured }: { animation: Animated.Value; label: string; type: 'book' | 'pencil' | 'room'; featured?: boolean }) {
  return (
    <Animated.View style={[styles.uniIconCard, featured && styles.uniIconCardFeatured, {
      transform: [{ translateY: animation.interpolate({ inputRange: [0, 1], outputRange: [0, featured ? -16 : -12] }) }],
    }]}>
...
function StatusBadge({ status, label }: { status: Status; label: string }) {
  return (
    <View style={[styles.statusBadge, status === 'free' && styles.freeBadge, status === 'current' && styles.currentBadge, status === 'full' && styles.fullBadge]}>
      <Text style={[styles.statusText, status === 'free' && styles.freeText, status === 'current' && styles.currentText, status === 'full' && styles.fullText]}>{label}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  logoMark: { width: 72, height: 72, borderRadius: 24, alignItems: 'center', justifyContent: 'center', marginBottom: 18, backgroundColor: colors.navy, shadowColor: colors.navy, shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.18, shadowRadius: 16, elevation: 5 },
